...
def main(args, config, build_model):
    ...
    for current_file in valid_files:
        ...
        if cfg.MODEL.get('FCOS',None) is not None:
        ...
        if cfg.TEST.AUG.ENABLED:
            ...
            evaluation_type = cfg.TEST.get('EVALUATION_TYPE',None)
            if evaluation_type is not None:
                evaluator = EVALUATOR.get(evaluation_type)()
            else:
                evaluator = None
            if teacher_model is not None:
                res = runner_decrator(RUNNERS.get(cfg.TRAINER.NAME)).test(cfg, model,teacher_model=teacher_model,evaluators=evaluator)
            else:
                res = runner_decrator(RUNNERS.get(cfg.TRAINER.NAME)).test(cfg, model,evaluators=evaluator,output_folder=config.OUTPUT_DIR)
