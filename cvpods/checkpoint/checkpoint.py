...
class Checkpointer(object):
    ...
    def has_checkpoint(self):
        ...
        """
        save_file = os.path.join(self.save_dir, self.model_name + "last_checkpoint")
    ...
    def step(self, iteration: int, **kwargs: Any):
        ...
        if self.period > 0 and (iteration + 1) % self.period == 0:
            if self.max_epoch is not None:
                ...
                if self.checkpointer.model_name.endswith("_iter_"):
                    self.checkpointer.model_name = self.checkpointer.model_name.split('_iter_')[0] + '_epoch_'
                ckpt_name = self.checkpointer.model_name + "{:04d}".format(curr_epoch)
            else:
                ckpt_name = self.checkpointer.model_name + "{:07d}".format(iteration + 1)
...
class DefaultCheckpointer(Checkpointer):
    ...
    def __init__(self, model, save_dir="", model_name="model_iter_",resume=False, *, save_to_disk=None, **checkpointables):
        """
        Args:
            model (nn.Module): model.
            save_dir (str): a directory to save and find checkpoints.
