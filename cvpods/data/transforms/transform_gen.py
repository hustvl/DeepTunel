...
class GeometricAugmentation(TransformGen):
    ...
    def __call__(self, img, annotations,**kwargs):
        if np.random.random() < self.prob:
            ...
            if self.record and annotations is not None:
                if "aug_info" not in annotations[0]:
                    annotations[0]["aug_info"] = []
                annotations[0]["aug_info"].append(self.get_aug_info(**magnitude))
                # annotations,valid_indx = self._filter_invalid(annotations, min_size=self.min_size)

            img,annotations = self.apply(img,annotations, **magnitude)

            if 'valid_indx' in annotations[0]:
                valid_indx = annotations[0]['valid_indx']
            else:
                valid_indx = np.ones(len(annotations),dtype=np.bool8)
            annotations,valid_indx = self._filter_invalid(annotations, min_size=self.min_size,valid_indx=valid_indx)
        else:
            valid_indx = np.ones(len(annotations),dtype=np.bool8)
        
        if 'valid_indx' not in annotations[0]:
            annotations[0].update({'valid_indx':valid_indx.tolist()})
        else:
            annotations[0]['valid_indx'] = (np.array(annotations[0]['valid_indx']) & valid_indx).tolist()
        
        if annotations is not None:
            for annotation in annotations:
                if "bbox" in annotation:
                    if annotation["bbox"].ndim == 2:
                        assert annotation["bbox"].shape[0]==1
...
##TODO
    def _filter_invalid(self, annotations,valid_indx,min_size=0):
        """Filter bboxes and masks too small or translated out of image."""
        if min_size is None:
            return annotations,np.ones(len(annotations),dtype=np.bool8)
        # valid_annotations = []
        # from pdb import set_trace
        # set_trace()
        valids = []
        for i,(annotation,valid_i) in enumerate(zip(annotations,valid_indx)):
            if not valid_i:
                valids.append(False)
                continue
        ...
        return annotations,np.array(valids)
