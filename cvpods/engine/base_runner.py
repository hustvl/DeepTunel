...
class RunnerBase:
    ...
    def after_step(self,start=0):
        for h in self._hooks[start:]:
            h.after_step()
...
class SimpleRunner(RunnerBase):
    ...
    def _write_metrics(
        ...
        if comm.is_main_process():
            ...
            if hasattr(self.model,'train_metic'):
                train_metic = {k:v.avg for k,v in self.model.train_metic.items()}
                metrics_dict.update(train_metic)
