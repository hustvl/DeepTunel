...
class StandardROIHeads(ROIHeads):
    ...
    def forward(
        ...
        if self.training:
            losses = self._forward_box(features_list, proposals,True,thre, targets)
    ...
    def _forward_box(
        self, features: List[torch.Tensor], proposals: List[Instances],pesudo_labels=False,thre=0.5,gt_instances=None
        ...
        if pesudo_labels:
            with torch.no_grad():
                pred_class_logits_per_img = pred_class_logits.split([len(x) for x in proposals])
                pred_proposal_deltas_per_img = pred_proposal_deltas.split([len(x) for x in proposals])
                for pred_cls,pred_bbox,proposal,gt_instance in zip(pred_class_logits_per_img,pred_proposal_deltas_per_img,proposals,gt_instances):
                    pred_cls_ = F.softmax(pred_cls,dim=1)
                    max_scores,max_arg = pred_cls_.max(dim=1)
                    gt_classes = proposal.gt_classes
                    # gt_boxes = gt_instance.gt_boxes
                    gt_classes[max_scores> thre] = max_arg[max_scores> thre]
