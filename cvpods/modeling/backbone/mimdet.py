...
from cvpods.utils.pos_embed import (
...
def build_mim_backbone(cfg):

    norm_layer = partial(nn.LayerNorm, eps=1e-5) if cfg.MODEL.BACKBONE.ENCODER.norm_layer =="LN" else None
    encoder = MIMDetEncoder(
                    img_size=cfg.MODEL.BACKBONE.ENCODER.img_size,
                    patch_size=cfg.MODEL.BACKBONE.ENCODER.patch_size,
                    in_chans=cfg.MODEL.BACKBONE.ENCODER.in_chans,
                    embed_dim=cfg.MODEL.BACKBONE.ENCODER.embed_dim,
                    depth=cfg.MODEL.BACKBONE.ENCODER.depth,
                    num_heads=cfg.MODEL.BACKBONE.ENCODER.num_heads,
                    mlp_ratio=cfg.MODEL.BACKBONE.ENCODER.mlp_ratio,
                    dpr=cfg.MODEL.BACKBONE.ENCODER.dpr,
                    norm_layer=norm_layer,
                    pretrained=cfg.MODEL.BACKBONE.pretrained,
                    checkpointing=cfg.MODEL.BACKBONE.ENCODER.checkpointing,
    )
    decoder = MIMDetDecoder(
        img_size=cfg.MODEL.BACKBONE.DECODER.img_size,
        patch_size=cfg.MODEL.BACKBONE.DECODER.patch_size,
        embed_dim=cfg.MODEL.BACKBONE.DECODER.embed_dim,
        decoder_embed_dim=cfg.MODEL.BACKBONE.DECODER.decoder_embed_dim,
        depth=cfg.MODEL.BACKBONE.DECODER.depth,
        num_heads=cfg.MODEL.BACKBONE.DECODER.num_heads,
        mlp_ratio=cfg.MODEL.BACKBONE.DECODER.mlp_ratio,
        dpr=cfg.MODEL.BACKBONE.DECODER.dpr,
        norm_layer=nn.LayerNorm,
        pretrained=cfg.MODEL.BACKBONE.pretrained,
        checkpointing=cfg.MODEL.BACKBONE.DECODER.checkpointing,
    )

    return MIMDetBackbone(encoder,
        decoder,
        cfg.MODEL.BACKBONE.sample_ratio,
        cfg.MODEL.BACKBONE.size_divisibility,
        _out_feature_channels=cfg.MODEL.BACKBONE._out_feature_channels,
        _out_feature_strides=cfg.MODEL.BACKBONE._out_feature_strides)