...
class SymmetricFocalLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        back_ce = torch.pow(1 - y_pred[:,0,:,:]+self.epsilon, self.gamma) * cross_entropy[:,0,:,:]
        back_ce =  (1 - self.delta) * back_ce

        fore_ce = torch.pow(1 - y_pred[:,1,:,:]+self.epsilon, self.gamma) * cross_entropy[:,1,:,:]
...
class AsymmetricFocalTverskyLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        fore_dice = (1-dice_class[:,1]) * torch.pow(1-dice_class[:,1]+self.epsilon, -self.gamma) 
