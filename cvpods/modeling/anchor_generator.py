...
class AnchorPoints(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.strides    = [x.stride for x in input_shape]
        self.offset     = cfg.MODEL.SHIFT_GENERATOR.OFFSET
        self.pyramid_levels = cfg.MODEL.P2PLike.IN_FEATURES
        self.row = cfg.MODEL.SHIFT_GENERATOR.ROW
        self.line = cfg.MODEL.SHIFT_GENERATOR.LINE
        self.pl = {'p3':3,'p4':4,'p5':5,'p6':6,'p7':7}
        if self.pyramid_levels is None:
            self.pyramid_levels = ['p3','p4','p5','p6','p7']
    ...
    def forward(self, feature):
        ...
        feature_shapes = [feature[x].shape[2:] for x in self.pyramid_levels]
        all_anchor_points = np.zeros((0, 2)).astype(np.float32)
        # get reference points for each level

        for idx, p in enumerate(self.pyramid_levels):
            
            anchor_points = generate_anchor_points(2**self.pl[p], row=self.row, line=self.line)
            shifted_anchor_points = shift(feature_shapes[idx], self.strides[idx], anchor_points)
