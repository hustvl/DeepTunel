...
class Instances:
    ...
    def draw(self,img,color,path=None,basename=None,line_size=2,with_label_and_scores=False,cat_ids2color=None,scores_fliter=None):
        ...
        if len(bboxes)!=0:
            ...
            if with_label_and_scores:
                ...
                for text,(left, top,_,_),valid in zip(texts,bboxes,valids):
                    if valid:
                        draw.text((left, top-20), text, (0,0,255),font=font)
                img = np.array(img)
